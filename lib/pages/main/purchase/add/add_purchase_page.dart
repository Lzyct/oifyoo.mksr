...
class _AddPurchasePageState extends State<AddPurchasePage> {
  AddPurchaseBloc _addPurchaseBloc;
  TransactionNumberPurchaseBloc _transactionNumberPurchaseBloc;
  ...
  void initState() {
    ...
    _addPurchaseBloc = BlocProvider.of(context);
    _transactionNumberPurchaseBloc = BlocProvider.of(context);
    _transactionNumberPurchaseBloc.transactionNumberPurchase();
  ...
  Widget build(BuildContext context) {
    return Parent(
      appBar: context.appBar(title: Strings.addPurchase),
      avoidBottomInset: true,
      child: MultiBlocListener(
        listeners: [
          BlocListener(
            cubit: _addPurchaseBloc,
            listener: (_, state) {
              switch (state.status) {
                case Status.LOADING:
                  {
          ...
          BlocListener(
            cubit: _transactionNumberPurchaseBloc,
            listener: (_, state) {
              switch (state.status) {
                case Status.LOADING:
                  {
                ...
                onPressed: () {
                  if (_formKey.currentState.validate()) {
                    if (_listSelectedProduct.isNotEmpty) {
                      ...
                      _addPurchaseBloc.addPurchase(_params);
                    } else {
